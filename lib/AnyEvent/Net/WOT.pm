...
has http_timeout     => ( is => 'rw', isa => 'Numeric', default => 1 );
has storage          => ( is => 'rw', isa => 'Object',  default => sub {AnyEvent::Net::WOT::Storage->new()});
has cache_bad_answer => ( is => 'rw', isa => 'Boolean', default => 0);
has default_answer   => ( is => 'rw', isa => 'HashRef', default => {0 => [50, 50], 4 => [50, 50]});


has caching_time => ( is => 'rw', isa => 'Numeric', default => 60*60*24 );
has bad_caching_time => ( is => 'rw', isa => 'Numeric', default => 60*30);


sub lookup {
	my ($self, $hosts, $cb_ret) = @_;
	$hosts = [$hosts] unless ref $host eq 'ARRAY';
	...
	$self->storage->get_info_by_hosts($hosts,sub {
		...
		http_get $usr, %{$self->param_for_http_req}, sub {
			my ($header, $body) = @_;
			my $result = {};
